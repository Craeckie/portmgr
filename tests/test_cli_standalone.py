...
def test_seal_unseal_round_trip_without_compose_file(tmp_path):
    ...
    # this only works because e/x are registered as 'standalone' and fall back
    ...
    unsealed = run_portmgr(["x"], cwd=tmp_path)
...
def test_non_standalone_command_skips_compose_less_dir(tmp_path):
    # 'm' (status) is not standalone; in a dir with no stacks it must produce no
    # per-directory output, confirming the fallback is opt-in.
    result = run_portmgr(["m"], cwd=tmp_path)
